...
class Siswa_model extends CI_Model
{
	...
	{
		// Mengatur SELECT statement
		$this->db->select('xx_jawaban.id_user, xx_jawaban.id_materi, MAX(xx_jawaban.Nilai) AS Nilai, xx_materi_.judul_materi,xx_materi_.kode_kelas, xx_users.nama');

		// Mengatur FROM statement dan JOIN dengan tabel xx_jawaban dan xx_materi_
		$this->db->from('xx_jawaban');
		$this->db->join('xx_materi_', 'xx_jawaban.id_materi = xx_materi_.id_materi');

		// Menambahkan JOIN dengan tabel user untuk mengambil nama
		$this->db->join('xx_users', 'xx_jawaban.id_user = xx_users.id_user');

		// Mengatur GROUP BY statement
		$this->db->group_by('xx_jawaban.id_user, xx_jawaban.id_materi');
				$query = $this->db->get();
		return $query->result_array();
	}
