...
class Home extends CI_Controller
{
	...
	{
		$data['title'] = 'infodaftar';
		$data['layout'] = 'home/infodaftar';
		$this->load->view('layout', $data);
	}
	...
	public function form()
{
    ...
    if ($this->input->post('submit')) {
        ...
        // Mendapatkan jenjang siswa dan kelas yang dipilih
        $siswaJenjang = $this->session->userdata('pendidikan');
        $kelasJenjang = $this->input->post('jenjang_kelas'); // Atau metode lain untuk mendapatkan jenjang kelas

        if ($siswaJenjang !== $kelasJenjang) {
            $this->session->set_flashdata('abort', 'Anda tidak bisa mendaftar kelas ini.');
            redirect('home/harga');
        } else {
            // Validasi input form
            $this->form_validation->set_rules('nama', 'Nama', 'trim|required|min_length[3]');
            $this->form_validation->set_rules('no_hp', 'No HP', 'trim|required|min_length[3]');
